...
public typealias fetchSuccess = (PHFetchResult<PHAsset>) -> ()
public typealias fetchFail = (NSError) -> ()



class ImageFetcher{
    
    private var success: fetchSuccess?
    private var fail: fetchFail?
    
    let libraryQueue = DispatchQueue(label: "com.google.qjadn0914")
    
    public func onSuccess(_ success: @escaping fetchSuccess) -> Self{
        self.success = success
        return self
    }
    
    public func onFail(_ fail: @escaping fetchFail) -> Self{
        self.fail = fail
        return self
    }
